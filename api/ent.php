<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ENT (Ear, Nose, Throat) & Head Neck Surgery | Prayag Hospital</title>

    <?php include 'header-links.php'; ?>

    <style>
        .doctor-card {
            width: 100% !important;
        }
    </style>
</head>

<body>

    <?php include 'header.php'; ?>

    <!-- Hero Section -->
    <section class="hero-section">
        <!-- Breadcrumb Navigation -->
        <div class="breadcrumb-wrapper">
            <div class="container">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <a href="index.php">
                                <i class="fas fa-home"></i>
                            </a>
                        </li>
                        <li class="breadcrumb-item">
                            <a href="#">Departments</a>
                        </li>
                        <li class="breadcrumb-item active" aria-current="page">
                            ENT (Ear, Nose, Throat)
                        </li>
                    </ol>
                </nav>
            </div>
        </div>

        <!-- Hero Content -->
        <div class="hero-content-wrapper">
            <div class="container">
                <div class="row align-items-center">
                    <!-- Left Column: Text Content -->
                    <div class="col-lg-7 col-md-12">
                        <h1 class="hero-main-title">
                            Advanced ENT & Head Neck Surgery Centre
                        </h1>
                        <p class="hero-subtitle" style="color:#000 !important">
                            Prayag Hospital offers comprehensive care for Ear, Nose, and Throat disorders. Our state-of-the-art facility provides advanced diagnostic and surgical treatments for sinusitis, hearing loss, voice disorders, and head & neck cancers.
                        </p>

                        <!-- CTA Buttons -->
                        <div class="hero-cta-buttons">
                            <a href="find-doctor.php" class="btn-book-appointment">Find Doctor</a>
                            <a href="#callback" class="btn-request-callback">
                                Request Callback
                            </a>
                        </div>

                        <!-- Hero Stats -->
                        <div class="hero-stats">
                            <div class="hero-stat-item">
                                <strong>Complete ENT Solutions</strong>
                                Expert care with advanced Micro-Ear Surgery, Endoscopic Sinus Surgery, and Voice Clinics.
                            </div>
                        </div>
                    </div>

                    <!-- Right Column: Hero Image -->
                    <div class="col-lg-5 col-md-12">
                        <div class="hero-image-wrapper">
                            <div class="hero-image-container">
                                <img src="../assets/images/ent-thumb.webp"
                                    alt="ENT Care at Prayag Hospital" class="hero-image">
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Table of Contents Navigation Strip -->
    <section class="toc-section">
        <div class="container-fluid px-0">
            <ul class="toc-nav">
                <li class="toc-nav-item">
                    <a href="#overview" class="toc-nav-link">Overview</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#conditions" class="toc-nav-link">Conditions</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#types-of-treatment" class="toc-nav-link">Treatments</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#why-choose-us" class="toc-nav-link">Why Choose Us</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#doctors" class="toc-nav-link">Doctors</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#patient-speak" class="toc-nav-link">Patient Speak</a>
                </li>
                <li class="toc-nav-item">
                    <a href="#faq" class="toc-nav-link">FAQ</a>
                </li>
            </ul>
        </div>
    </section>

    <!-- Features Section -->
    <section class="features-section">
        <div class="container">
            <div class="row g-4">
                <!-- Feature 1 -->
                <div class="col-lg-3 col-md-6 col-12">
                    <div class="feature-box">
                        <div class="feature-icon">
                            <i class="fas fa-deaf"></i>
                        </div>
                        <div class="feature-content">
                            <h3 class="feature-title">Micro Ear Surgery</h3>
                        </div>
                    </div>
                </div>

                <!-- Feature 2 -->
                <div class="col-lg-3 col-md-6 col-12">
                    <div class="feature-box">
                        <div class="feature-icon">
                            <i class="fas fa-head-side-mask"></i>
                        </div>
                        <div class="feature-content">
                            <h3 class="feature-title">Sinus Endoscopy</h3>
                        </div>
                    </div>
                </div>

                <!-- Feature 3 -->
                <div class="col-lg-3 col-md-6 col-12">
                    <div class="feature-box">
                        <div class="feature-icon">
                            <i class="fas fa-microphone-alt"></i>
                        </div>
                        <div class="feature-content">
                            <h3 class="feature-title">Voice Clinic</h3>
                        </div>
                    </div>
                </div>

                <!-- Feature 4 -->
                <div class="col-lg-3 col-md-6 col-12">
                    <div class="feature-box">
                        <div class="feature-icon">
                            <i class="fas fa-balance-scale"></i>
                        </div>
                        <div class="feature-content">
                            <h3 class="feature-title">Vertigo Clinic</h3>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Overview Section -->
    <section class="overview-section" id="overview">
        <div class="container">
            <div class="overview-content">
                <h2 class="overview-heading">Expert ENT Care at Prayag Hospital</h2>
                <p class="overview-text">
                    The Department of ENT (Otorhinolaryngology) at Prayag Hospital provides comprehensive medical and surgical management for disorders of the Ear, Nose, Throat, and Head & Neck region. We cater to all age groups, offering specialized care for paediatric ENT issues as well as adult cases.
                </p>
                <p class="overview-text">
                    Our department is equipped with the latest diagnostic and surgical technologies, including operating microscopes for delicate ear surgeries and high-definition endoscopes for minimally invasive nasal and sinus procedures. We focus on providing precise diagnosis and effective treatment plans tailored to each patient.
                </p>
                <p class="overview-text">
                    From routine procedures like tonsillectomy and adenoidectomy to complex surgeries like Cochlear Implants, skull base surgeries, and extensive head & neck cancer resections/reconstructions, our team of highly experienced ENT surgeons ensures the best possible outcomes with minimal recovery time.
                </p>
            </div>

            <div class="overview-image-wrapper">
                <img src="../assets/images/ent-main.webp"
                    alt="ENT Department" class="overview-image">
            </div>

            <div class="overview-highlights">
                <div class="overview-highlight">
                    <i class="fas fa-microscope"></i>
                    <h4>Microsurgery</h4>
                    <p>High-precision microscopic surgeries for restoring hearing and treating ear infections.</p>
                </div>
                <div class="overview-highlight">
                    <i class="fas fa-video"></i>
                    <h4>Video Endoscopy</h4>
                    <p>Advanced endoscopic procedures for diagnosis and minimally invasive treatment of nasal/throat issues.</p>
                </div>
                <div class="overview-highlight">
                    <i class="fas fa-assistive-listening-systems"></i>
                    <h4>Audiology</h4>
                    <p>Comprehensive hearing assessment and rehabilitation services including hearing aids.</p>
                </div>
            </div>
        </div>
    </section>

    <!-- Conditions We Treat Section -->
    <section class="conditions-section" id="conditions">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Conditions We Treat</h2>
                <p class="section-subtitle">
                    We manage a wide spectrum of ENT disorders with expertise and advanced care.
                </p>
            </div>

            <div class="row g-3">
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Chronic Sinusitis</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Tonsillitis & Adenoids</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Hearing Loss</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Ear Infections (Otitis Media)</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Vertigo & Dizziness</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Voice Disorders (Hoarseness)</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Snoring & Sleep Apnea</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Deviated Nasal Septum (DNS)</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Nasal Polyps</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Head & Neck Tumors</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Thyroid Disorders</h3>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-sm-6 col-12">
                    <div class="condition-card">
                        <h3 class="condition-name">Foreign Bodies in Ear/Nose/Throat</h3>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Types of Treatment Section -->
    <section class="treatment-section" id="types-of-treatment">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Treatments & Procedures</h2>
                <p class="section-subtitle">
                    Advanced surgical and medical treatments for all ENT conditions.
                </p>
            </div>

            <div class="row g-4">
                <!-- Treatment Card 1 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-file-medical-alt"></i>
                        </div>
                        <h3 class="treatment-card-title">FESS (Sinus Surgery)</h3>
                        <p class="treatment-card-description">
                            Functional Endoscopic Sinus Surgery is a minimally invasive technique to treat chronic sinusitis and nasal polyps.
                        </p>
                    </div>
                </div>

                <!-- Treatment Card 2 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-deaf"></i>
                        </div>
                        <h3 class="treatment-card-title">Tympanoplasty</h3>
                        <p class="treatment-card-description">
                            Microsurgical procedure to repair perforated eardrums and improve hearing levels.
                        </p>
                    </div>
                </div>

                <!-- Treatment Card 3 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-child"></i>
                        </div>
                        <h3 class="treatment-card-title">Tonsillectomy & Adenoidectomy</h3>
                        <p class="treatment-card-description">
                             Removal of tonsils and adenoids, often performed for recurrent infections or breathing issues in children/adults.
                        </p>
                    </div>
                </div>

                <!-- Treatment Card 4 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-head-side-mask"></i>
                        </div>
                        <h3 class="treatment-card-title">Septoplasty</h3>
                        <p class="treatment-card-description">
                            Surgical correction of a deviated nasal septum to improve airflow and breathing.
                        </p>
                    </div>
                </div>

                <!-- Treatment Card 5 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-microphone"></i>
                        </div>
                        <h3 class="treatment-card-title">Microlaryngeal Surgery</h3>
                        <p class="treatment-card-description">
                            Precise surgery on the vocal cords to treat voice disorders, cysts, and polyps using a microscope.
                        </p>
                    </div>
                </div>

                <!-- Treatment Card 6 -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="treatment-card">
                        <div class="treatment-card-icon">
                            <i class="fas fa-dna"></i>
                        </div>
                        <h3 class="treatment-card-title">Head & Neck Surgery</h3>
                        <p class="treatment-card-description">
                            Comprehensive management of thyroid nodules, salivary gland tumors, and other head and neck masses.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Why Choose Us Section -->
    <section class="why-choose-section" id="why-choose-us">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Why Choose Prayag for ENT?</h2>
                <p class="section-subtitle">
                    Leading ENT care combining expertise with the latest technology for superior patient outcomes.
                </p>
            </div>

            <!-- Why Points Grid -->
            <div class="why-points-grid">
                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-user-md"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Expert ENT Surgeons</h4>
                        <p>Highly qualified consultants with years of experience in micro-ear and endoscopic surgeries.</p>
                    </div>
                </div>

                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-microscope"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Advanced Equipment</h4>
                        <p>Equipped with Zeiss microscopes, Karl Storz endoscopes, and advanced audiology labs.</p>
                    </div>
                </div>

                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-balance-scale"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Specialty Clinics</h4>
                        <p>Dedicated Vertigo Clinic, Voice Clinic, and Sleep Apnea Clinic for focused treatment.</p>
                    </div>
                </div>

                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-hospital-user"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Comprehensive Care</h4>
                        <p>Holistic management from diagnosis to surgery and rehabilitation (speech therapy/audiology).</p>
                    </div>
                </div>

                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-child"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Paediatric ENT</h4>
                        <p>Specialized compassionate care for children with ENT problems like glue ear, adenoids, etc.</p>
                    </div>
                </div>

                <div class="why-point">
                    <div class="why-point-icon">
                        <i class="fas fa-clock"></i>
                    </div>
                    <div class="why-point-content">
                        <h4>Emergency Services</h4>
                        <p>24/7 management of ENT emergencies like foreign bodies, nose bleeds (epistaxis), and breathing difficulty.</p>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <!-- Doctors Section -->
    <section class="doctors-section" id="doctors">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Our ENT Specialists</h2>
                <p class="section-subtitle">
                    Meet our team of experienced Ear, Nose, and Throat surgeons.
                </p>
            </div>

            <div class="row g-4 justify-content-center">
                <?php include 'doctors-data.php'; ?>
                <?php 
                $ent_doctors = array_filter($doctors, function($doctor) {
                    return $doctor['specialty_code'] === 'ent' || $doctor['specialty_code'] === 'otorhinolaryngology';
                });
                
                // Fallback for demo if no docs found
                if (empty($ent_doctors)) {
                     // Using General Surgery docs as placeholder for demo if ENT specifically not found in data
                    $ent_doctors = array_filter($doctors, function($doctor) {
                         return $doctor['specialty_code'] === 'general-surgery';
                    });
                     $ent_doctors = array_slice($ent_doctors, 0, 3);
                }

                foreach ($ent_doctors as $doctor): 
                ?>
                <!-- Doctor Card -->
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="doctor-card">
                        <div class="doctor-card-inner">
                            <div class="doctor-card-top">
                                <div class="doctor-image-container">
                                    <img src="<?php echo !empty($doctor['image_small']) ? $doctor['image_small'] : $doctor['image']; ?>"
                                        alt="<?php echo $doctor['name']; ?>" class="doctor-image">
                                </div>
                                <div class="doctor-info">
                                    <h3 class="doctor-name"><?php echo $doctor['name']; ?></h3>
                                    <p class="doctor-designation"><?php echo $doctor['specialty']; ?></p>
                                    <p class="doctor-experience">
                                        <span class="experience-label">Experience:</span>
                                        <span class="experience-value"><?php echo $doctor['experience']; ?> Years</span>
                                    </p>
                                </div>
                            </div>
                            <div class="doctor-actions">
                                <a href="doctor-profile.php?id=<?php echo $doctor['id']; ?>" class="btn-view-profile">View Profile</a>
                                <a href="<?php echo isset($doctor['slug']) ? $doctor['slug'] : $doctor['id']; ?>.php#book-appointment" class="btn-book-now">Book Apt.</a>
                            </div>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
    </section>

    <!-- Patient Speak Section (Testimonials) -->
    <section class="patient-speak-section" id="patient-speak">
        <div class="container">
            <div class="section-header-center">
                <h2 class="section-title-dark">Patient Success Stories</h2>
                <p class="section-subtitle-gray">
                    Hear from our patients who have found relief from their ENT troubles.
                </p>
            </div>

            <div class="testimonials-slider-wrapper">
                <button class="testimonial-nav testimonial-nav-prev" aria-label="Previous">
                    <i class="fas fa-chevron-left"></i>
                </button>

                <div class="testimonials-slider-container">
                    <div class="testimonials-slider">
                        <!-- Testimonial 1 -->
                        <div class="testimonial-card">
                            <div class="testimonial-thumbnail">
                                <img src="https://via.placeholder.com/400x250/e3f2fd/1565c0?text=Sinus+Relief"
                                    alt="Sinus Patient">
                                <div class="play-button">
                                    <i class="fas fa-play"></i>
                                </div>
                            </div>
                            <div class="testimonial-overlay">
                                <p class="testimonial-text">"I suffered from chronic sinusitis for years. The endoscopic surgery at Prayag Hospital was painless and now I can breathe freely."</p>
                                <h4 class="testimonial-name">Ms. R. Verma</h4>
                                <p class="testimonial-procedure">FESS Surgery</p>
                            </div>
                        </div>

                        <!-- Testimonial 2 -->
                        <div class="testimonial-card">
                            <div class="testimonial-thumbnail">
                                <img src="https://via.placeholder.com/400x250/e3f2fd/1565c0?text=Better+Hearing"
                                    alt="Hearing Patient">
                                <div class="play-button">
                                    <i class="fas fa-play"></i>
                                </div>
                            </div>
                            <div class="testimonial-overlay">
                                <p class="testimonial-text">"The tympanoplasty surgery restored my hearing significantly. I am very grateful to the ENT team for their care."</p>
                                <h4 class="testimonial-name">Mr. K. Singh</h4>
                                <p class="testimonial-procedure">Ear Surgery</p>
                            </div>
                        </div>

                        <!-- Testimonial 3 -->
                        <div class="testimonial-card">
                            <div class="testimonial-thumbnail">
                                <img src="https://via.placeholder.com/400x250/e3f2fd/1565c0?text=Voice+Restored" alt="Voice Patient">
                                <div class="play-button">
                                    <i class="fas fa-play"></i>
                                </div>
                            </div>
                            <div class="testimonial-overlay">
                                <p class="testimonial-text">"I had lost my voice due to a vocal polyp. The microsurgery was successful and my voice is back to normal."</p>
                                <h4 class="testimonial-name">Mrs. A. Sharma</h4>
                                <p class="testimonial-procedure">Microlaryngeal Surgery</p>
                            </div>
                        </div>
                    </div>
                </div>

                <button class="testimonial-nav testimonial-nav-next" aria-label="Next">
                    <i class="fas fa-chevron-right"></i>
                </button>
            </div>
        </div>
    </section>

    <!-- FAQ Section -->
    <section class="faq-section" id="faq">
        <div class="container">
            <div class="section-header">
                <h2 class="section-title">Frequently Asked Questions</h2>
                <p class="section-subtitle">
                    Common questions about Ear, Nose, and Throat problems.
                </p>
            </div>

            <div class="row">
                <div class="col-lg-10 mx-auto">
                    <div class="accordion" id="faqAccordion">
                        <!-- FAQ 1 -->
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading1">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse1" aria-expanded="true" aria-controls="faqCollapse1">
                                    When should I see an ENT specialist?
                                </button>
                            </h2>
                            <div id="faqCollapse1" class="accordion-collapse collapse show"
                                aria-labelledby="faqHeading1" data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    You should see an ENT specialist if you have persistent ear pain, hearing loss, chronic blocked nose, sinus pain, sore throat sticking for weeks, voice changes, dizziness, or a lump in the neck.
                                </div>
                            </div>
                        </div>

                        <!-- FAQ 2 -->
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading2">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse2" aria-expanded="false" aria-controls="faqCollapse2">
                                    What is Endoscopic Sinus Surgery?
                                </button>
                            </h2>
                            <div id="faqCollapse2" class="accordion-collapse collapse" aria-labelledby="faqHeading2"
                                data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    It is a minimally invasive surgery performed through the nostrils using an endoscope to open up sinus passages and remove fluid or polyps. There are no external cuts or scars on the face.
                                </div>
                            </div>
                        </div>

                         <!-- FAQ 3 -->
                         <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading3">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse3" aria-expanded="false" aria-controls="faqCollapse3">
                                    Is tonsil surgery safe for children?
                                </button>
                            </h2>
                            <div id="faqCollapse3" class="accordion-collapse collapse" aria-labelledby="faqHeading3"
                                data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    Yes, tonsillectomy is a very common and safe procedure often performed on children who suffer from recurrent tonsillitis or sleep apnea affecting their health. Recovery is usually quick, within a week to 10 days.
                                </div>
                            </div>
                        </div>

                        <!-- FAQ 4 -->
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading4">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse4" aria-expanded="false" aria-controls="faqCollapse4">
                                    How do I know if I need a hearing aid?
                                </button>
                            </h2>
                            <div id="faqCollapse4" class="accordion-collapse collapse" aria-labelledby="faqHeading4"
                                data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    If you struggle to hear conversations in noisy places, frequently ask people to repeat themselves, or turn up the TV volume louder than others, you should get a hearing test (audiometry) to determine if a hearing aid will help.
                                </div>
                            </div>
                        </div>

                        <!-- FAQ 5 -->
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading5">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse5" aria-expanded="false" aria-controls="faqCollapse5">
                                    What causes vertigo?
                                </button>
                            </h2>
                            <div id="faqCollapse5" class="accordion-collapse collapse" aria-labelledby="faqHeading5"
                                data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    Vertigo is often caused by inner ear problems like BPPV (Benign Paroxysmal Positional Vertigo), infection, or Meniere's disease. Our specialists can diagnose the cause and recommend treatments like Epley maneuver or medication.
                                </div>
                            </div>
                        </div>

                        <!-- FAQ 6 -->
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="faqHeading6">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#faqCollapse6" aria-expanded="false" aria-controls="faqCollapse6">
                                    How are voice disorders treated?
                                </button>
                            </h2>
                            <div id="faqCollapse6" class="accordion-collapse collapse" aria-labelledby="faqHeading6"
                                data-bs-parent="#faqAccordion">
                                <div class="accordion-body">
                                    Treatment depends on the cause (nodules, polyps, or paralysis). Options range from speech therapy and voice rest to microlaryngeal surgery (phonomicrosurgery) to restore vocal cord function.
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <?php include 'footer.php'; ?>

    <?php include 'footer-links.php'; ?>

    <script>
        // Testimonials Slider Logic
        const slider = document.querySelector('.testimonials-slider');
        const prevBtn = document.querySelector('.testimonial-nav-prev');
        const nextBtn = document.querySelector('.testimonial-nav-next');
        const cards = document.querySelectorAll('.testimonial-card');
        
        if(slider && cards.length > 0) {
            let cardWidth = cards[0].offsetWidth + 24; // Width + gap
            
            nextBtn.addEventListener('click', () => {
                slider.scrollBy({ left: cardWidth, behavior: 'smooth' });
            });

            prevBtn.addEventListener('click', () => {
                slider.scrollBy({ left: -cardWidth, behavior: 'smooth' });
            });
        }
    </script>
</body>

</html>

